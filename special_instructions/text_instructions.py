...
def draw_big_fermata(length_weights, size_factor=1):
    ...
    length = chance.rand.weighted_rand(length_weights, do_round=True)
    ...
    score = lily.lilypond_file.LilypondFile()
    ...
    output_file_name = "fermata_" + str(shared.Globals.FileNameIndex)
...
def draw_boxed_text(text_string, line_width=45):
    ...
    output_file_name = "text_string_" + str(shared.Globals.FileNameIndex)
    output_png = score.save_and_render(output_file_name, view_image=False, autocrop=True, delete_ly=False)
    shared.Globals.FileNameIndex += 1
    return output_png
