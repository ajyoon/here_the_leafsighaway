...
from chance import network, nodes, prebuilt_networks, rand
...
from document_tools import tools
...
class Paragraph:
    ...
    def tweak_indent(self):
        ...
        if self.left_indent > (Globals.DocWorkingWidth - self.right_indent - Globals.MinColumnWidth):
            self.left_indent = (Globals.DocWorkingWidth - self.right_indent - Globals.MinColumnWidth)
        if self.left_indent > Globals.DocWorkingWidth - Globals.MinColumnWidth:
            self.left_indent = Globals.DocWorkingWidth - Globals.MinColumnWidth - 25
        if Globals.DocWorkingWidth - self.left_indent - self.right_indent < Globals.MinColumnWidth:
            self.left_indent -= Globals.MinColumnWidth
            self.right_indent -= Globals.MinColumnWidth
        if self.left_indent < 0:
            self.left_indent = random.randint(0, 15)
            self.right_indent -= self.left_indent
        if Globals.DocWorkingWidth - self.right_indent < self.left_indent:
            self.right_indent = Globals.DocWorkingWidth - self.left_indent - Globals.MinColumnWidth - 1
    ...
    def add_to_doc(self, flowable_list, base_style):
        ...
        if paragraph_string == '':
            # paragraph_string = '---'
            flowable_list.append(reportlab.platypus.Spacer(16, Globals.FontSize))
            return
...
class InlineImage:
    def __init__(self, file_path, height=None, left_indent=0, right_indent=0,
        ...
        self.width = round((72.0 * image_pix_width) / Globals.ImageDPI, 3)
        self.height = round((72.0 * image_pix_height) / Globals.ImageDPI, 3)
    ...
    def tweak_indent(self):
        ...
        if self.width > Globals.DocWorkingWidth:
        ...
        if self.alignment == 'LEFT':
            self.right_indent = 0
        if self.left_indent > Globals.DocWorkingWidth - self.width - 20:
            self.left_indent = (Globals.DocWorkingWidth - self.width - 20)
        if self.right_indent > Globals.DocWorkingWidth - self.width + self.left_indent:
            self.right_indent = Globals.DocWorkingWidth - self.width + self.left_indent
        if self.left_indent < 0:
            self.left_indent = 0
        if self.right_indent < 0:
            self.right_indent = 0
...
class DynamicScribe:
    def __init__(self, source, relationship_weights=None, instrument_name=None, allow_self_links=False,
        ...
        tools.register_font(Globals.FontName)
    ...
    def render(self, output_file, open_when_finished=False, update_frequency=25):
        ...
        for i in range(len(self.item_list)):
            ...
            if left_indent > Globals.DocWorkingWidth - Globals.MinColumnWidth:
                left_indent = Globals.DocWorkingWidth - Globals.MinColumnWidth - 30
            if left_indent < 0:
                left_indent = random.randint(0, 30)
                right_indent -= left_indent
            if Globals.DocWorkingWidth - right_indent < left_indent + Globals.MinColumnWidth:
                right_indent = Globals.DocWorkingWidth - left_indent - Globals.MinColumnWidth - 1
            if right_indent < 0:
            ...
            if isinstance(self.item_list[i], nodes.Word):
                ...
                if current_line.contents is not None:
                        ...
                        current_line.add_to_doc(self.flowable_list, self.base_paragraph_style)
                        current_line = Paragraph()
                self.flowable_list.append(reportlab.platypus.Spacer(16, Globals.FontSize))
        ...
        def build_header_canvas(canvas, doc):
            ...
            canvas.saveState()
            canvas.setFont(Globals.FontName, 11)
...
class InstrumentScribe:
    ...
    def render(self, steps, output_file, open_when_finished=False, update_frequency=25):
        ...
        for i in range(steps):
            ...
            if Globals.DocWorkingWidth - right_indent < left_indent:
                right_indent = Globals.DocWorkingWidth - left_indent - 30
            if right_indent < 0:
                right_indent = 0
            dummy_align = rand.weighted_rand(
                [rand.Weight('LEFT', 70),
